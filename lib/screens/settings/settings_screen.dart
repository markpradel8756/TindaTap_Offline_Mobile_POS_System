...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _exportBackup() async {
    try {
      final path = await context.read<SettingsProvider>().exportBackup();
      await Share.shareXFiles([XFile(path)], text: 'TindaTap backup file');
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _importBackup() async {
    try {
      final XTypeGroup dbGroup =
          XTypeGroup(label: 'database', extensions: ['db']);
      final XFile? file =
          await openFile(acceptedTypeGroups: <XTypeGroup>[dbGroup]);
      if (file == null) return;
      await context.read<SettingsProvider>().importBackup(file.path);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            onTap: () async {
                              final choice = await showDialog<String?>(
                                ...
                                builder: (context) => SimpleDialog(
                                  ...
                                  children: AppConstants.supportedCurrencyCodes
                                      .map((code) {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ListTile(
                            ...
                            subtitle: const Text('Backup all your store data'),
                            trailing: const Icon(Icons.chevron_right),
                            onTap: _exportBackup,
                          ),
                          const SizedBox(height: 8),
                          ...
                          if (settings.qrCodes.isEmpty)
                            ...
                            ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                return _buildQrTile(settings.qrCodes[index]);
                              },
                            ),
                        ],
                      ),
